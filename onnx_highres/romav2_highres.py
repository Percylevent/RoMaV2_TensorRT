...
class RoMaV2HighRes(nn.Module):
    def __init__(self, weights_path: str, H_lr: int = 512, W_lr: int = 512, H_hr: int = 768, W_hr: int = 768, bidirectional: bool = True):
        super().__init__()
        ...
        print(f"Initializing RoMaV2HighRes with: LR={H_lr}x{W_lr}, HR={H_hr}x{W_hr}, Bidirectional={bidirectional}")
    ...
    def forward(
        ...
        # Feature extraction on low-res images
        f_A = self.f(img_A_lr)
        f_B = self.f(img_B_lr)
        ...
        # Refinement: Two stages (Low-Res, then High-Res)
        # Stage 0: Low-res refinement
        # Stage 1: High-res refinement
        for stage, (img_A, img_B) in enumerate(
            ...
            # Loop over patch sizes explicitly: 4 → 2 → 1
            for patch_size_str in ["4", "2", "1"]:
                refiner = self.refiners[patch_size_str]
                patch_size = int(patch_size_str)
                
                # zero_out_precision logic: stage==1 and patch_size==4
                zero_out_precision = (stage == 1 and patch_size == 4)
