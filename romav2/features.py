...
class Descriptor:
    ...
    def __new__(cls, cfg: Cfg) -> nn.Module:
        ...
        match cfg.name:
            case "dinov3_vitl16":
                normalizer = imagenet
                # Load Dino v3 from local directory instead of torch.hub
                # The Dino v3 weights are already included in romav2.pt
                # We just need the architecture
                import sys
                from pathlib import Path

                # Get path to local dinov3
                local_dino_path = Path(__file__).parent.parent.parent / "facebookresearch_dinov3_adc254450203739c8149213a7a69d8d905b4fcfa"
                if local_dino_path.exists() and str(local_dino_path) not in sys.path:
                    sys.path.insert(0, str(local_dino_path))

                # Import hubconf from local dinov3
                import hubconf
                dinov3_vitl16: nn.Module = hubconf.dinov3_vitl16(pretrained=False).to(device)
...
class VGG(nn.Module):
    def forward(self, x):
        x = imagenet(x)
        # Use simple float32 context or disable autocast
