...
def visualize_onnx(size: int, bidirectional: bool):
    H_lr = size
    W_lr = size
    ...
    im1_pil_resized = im1_pil.resize((W_lr, H_lr), Image.BICUBIC)
    im2_pil_resized = im2_pil.resize((W_lr, H_lr), Image.BICUBIC)
    
    im1_np = np.array(im1_pil_resized).transpose(2, 0, 1).astype(np.float32) / 255.0
    im2_np = np.array(im2_pil_resized).transpose(2, 0, 1).astype(np.float32) / 255.0
    
    im1 = im1_np[None]  # Add batch dim
    im2 = im2_np[None]
    ...
    im1_tensor = torch.from_numpy(im1).cuda()
    im2_tensor = torch.from_numpy(im2).cuda()
    ...
    white_im = torch.ones((H_lr, 2 * W_lr), device='cuda')
    vis_im = overlap * warp_im + (1 - overlap) * white_im

    # Save
    out_path = current_dir / f"vis_{size}_{bi_str}.jpg"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Visualize RoMaV2 Low-Res ONNX")
    ...
    visualize_onnx(size=args.size, bidirectional=args.bidirectional)
